...
test.describe('Title Persistence', () => {
  test('should persist title after page refresh', async ({ cleanPage, helpers }) => {
  ...
  test('should handle very long titles without breaking UI', async ({ cleanPage, helpers }) => {
    // Create a test conversation
    const conversationId = await helpers.createTestConversation(
      'Short Title',
      [{ role: 'user', content: 'Test message' }]
    ...
    const longTitle = 'This is a very long conversation title that exceeds normal length expectations and should be handled gracefully by the UI without breaking the layout or causing overflow issues. It contains more than two hundred characters to test edge cases.';
    ...
    const titleElement = await conversationItem.locator('[data-testid="conversation-title"]').first();
  ...
  test('should persist final title after rapid updates', async ({ cleanPage, helpers }) => {
    // Create a test conversation
    const conversationId = await helpers.createTestConversation(
      'Original Title',
      [{ role: 'user', content: 'Test message' }]
  ...
  test('should handle title persistence with special characters', async ({ cleanPage, helpers }) => {
    // Create a test conversation
    const conversationId = await helpers.createTestConversation(
      'Simple Title',
      [{ role: 'user', content: 'Test message' }]
    ...
    const specialTitle = 'Title with "quotes" & <tags> and émojis 🎉 and symbols: @#$%^&*()';
  ...
  test('should persist title across multiple refresh cycles', async ({ cleanPage, helpers }) => {
    // Create a test conversation
    const conversationId = await helpers.createTestConversation(
      'Original Title',
      [{ role: 'user', content: 'Test message' }]
  ...
  test('should maintain title persistence when other conversations are modified', async ({ cleanPage, helpers }) => {
