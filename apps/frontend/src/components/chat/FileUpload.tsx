...
import { SecurityScanner, type SecurityScanResult } from '../../utils/security.js';
...
const DANGEROUS_FILE_EXTENSIONS = new Set(['.exe', '.bat', '.cmd', '.scr', '.pif', '.com', '.jar']);
...
const FileUploadComponent = ({
  ...
  const updateUploadingFile = useCallback((id: string, updater: (file: UploadingFile) => UploadingFile): void => {
    setUploadingFiles(prev => prev.map(uploadingFile => (uploadingFile.id === id ? updater(uploadingFile) : uploadingFile)));
  }, []);
   ...
   */
  const performSecurityScan = useCallback(async (file: File): Promise<SecurityScanResult> => {
    try {
  ...
  const handleFiles = useCallback(async (files: FileList): Promise<void> => {
    ...
    const firstInvalid = validationResults.find(validation => !validation.result.valid);
    if (firstInvalid) {
      onError(firstInvalid.result.error ?? t('fileUpload.invalidFile'));
      return;
    }

    const initialUploads: UploadingFile[] = validationResults.map(({ file }) => ({
      file,
    ...
    try {
      for (const uploading of initialUploads) {
        updateUploadingFile(uploading.id, (uf) => ({ ...uf, progress: 10, status: 'scanning' }));

        const preview = await generatePreview(uploading.file);
        updateUploadingFile(uploading.id, (uf) => ({ ...uf, progress: 50, preview }));
        ...
        if (!securityScan.safe) {
          updateUploadingFile(uploading.id, (uf) => ({
            ...
            progress: 100,
            status: 'error',
            error: t('fileUpload.security.threatDetected', { threats: securityScan.threats.join(', ') }),
            securityScan,
          }));
          continue;
        }
      ...
      setUploadingFiles(prev => prev.map(uf => ({ ...uf, status: 'error', _error: errorMessage })));
  ...
  }, [disabled, maxFiles, validateFile, onError, onFilesSelected, generatePreview, performSecurityScan, t, updateUploadingFile]);
      ...
      {uploadingFiles.length > 0 && (
        <div className="upload-progress">
          ...
          {uploadingFiles.map((uploadingFile) => {
            ...
            const hasConfidence = typeof confidence === 'number' && Number.isFinite(confidence) && confidence > 0;
            const errorMessage = typeof uploadingFile.error === 'string' && uploadingFile.error.trim().length > 0
              ? uploadingFile.error
              : null;
            const previewContent = typeof uploadingFile.preview === 'string' && uploadingFile.preview.length > 0
            ...
            return (
              ...
              <div className="upload-info">
                ...
                {showPreview && uploadingFile.status === 'success' && (
                  <button
                    ...
                    aria-label={t('fileUpload.previewFile', { name: uploadingFile.file.name })}
                  >
                    👁️
                  </button>
                )}
              ...
              <div className="upload-status">
                {(uploadingFile.status === 'uploading' || uploadingFile.status === 'scanning') && (
                ...
                {uploadingFile.status === 'success' && (
                  <div className="status-icon success" title={t('fileUpload.uploadSuccess')}>
                    ✓
                  </div>
                )}
                
                {uploadingFile.status === 'error' && (
                  <div className="status-icon error" title={t('fileUpload.uploadError')}>
                    ✗
                  </div>
                )}

                {uploadingFile.status === 'scanning' && (
                  <div className="status-icon scanning" title={t('fileUpload.securityScanning')}>
                    🔍
                  </div>
                )}
              </div>
              ...
              {uploadingFile.securityScan?.safe === true && (
                <div className="security-status safe">
                  ...
                  <span className="security-text">
                    ...
                    {hasConfidence ? ` (${Math.round(confidence * 100)}%)` : ''}
                  </span>
                </div>
              )}

              {previewContent !== null && uploadingFile.status === 'success' && (
                <div className="file-preview-thumbnail">
                  {uploadingFile.file.type.startsWith('image/') ? (
                    <img 
                      src={previewContent} 
                      alt={t('fileUpload.previewAlt', { name: uploadingFile.file.name })}
