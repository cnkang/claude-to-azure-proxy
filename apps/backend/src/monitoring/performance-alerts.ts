...
export class PerformanceAlertSystem extends EventEmitter {
  ...
  private collectMetrics(): void {
    ...
    const recentGCEvents = this.gcEvents.filter(
      (event) => now.getTime() - event.timestamp < 300000 // Last 5 minutes
    );
    ...
    const metrics: PerformanceMetrics = {
      ...
      gcMetrics: {
        totalDuration: recentGCEvents.reduce(
        ...
        averageDuration:
          recentGCEvents.length > 0
            ? recentGCEvents.reduce((sum, event) => sum + event.duration, 0) /
              recentGCEvents.length
            : 0,
      },
      throughput: {
...
export function getPerformanceAlertSystem(
  configs?: AlertConfig[]
