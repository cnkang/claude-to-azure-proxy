...
This test script (`test-azure-direct.ts`) makes direct axios calls to the Azure OpenAI Responses API to isolate and debug the "canceled" error issue discovered during Task 4.7 validation. It bypasses the streaming service complexity to test the API directly with minimal configuration.
...
1. **Test 1 vs Test 2**: If Test 1 succeeds but Test 2 fails, the AbortController presence is causing the issue
2. **Test 2 vs Test 3**: If Test 3 succeeds but Test 2 fails, the timeout configuration may interact poorly with AbortController
