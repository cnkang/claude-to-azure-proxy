...
export class DataIntegrityService {
  ...
  private async detectOrphanedMessagesLocalStorage(
    ...
    try {
      ...
      frontendLogger.warn(
        'Failed to check localStorage for orphaned messages',
        {
          error: error instanceof Error ? error : new Error(String(error)),
        }
      );
    ...
    try {
      ...
      for (const messageId of idsToClean) {
        try {
          ...
          frontendLogger.warn(
            'Failed to remove orphaned message from localStorage',
            {
              error: error instanceof Error ? error : new Error(String(error)),
  ...
  private validateConversationSchema(conversation: Conversation): string[] {
    ...
    if (!conversation.id || typeof conversation.id !== 'string') {
      errors.push(
        `Conversation ${conversation.id ?? 'unknown'}: Invalid or missing ID`
      );
    }
    ...
    if (!conversation.sessionId || typeof conversation.sessionId !== 'string') {
      errors.push(
        `Conversation ${conversation.id}: Invalid or missing sessionId`
      );
    }

    if (
      !conversation.selectedModel ||
      typeof conversation.selectedModel !== 'string'
    ) {
      errors.push(
        `Conversation ${conversation.id}: Invalid or missing selectedModel`
      );
    }
    ...
    if (
      ...
      isNaN(conversation.createdAt.getTime())
    ) {
    ...
    if (
      ...
      isNaN(conversation.updatedAt.getTime())
    ) {
    ...
    if (!Array.isArray(conversation.messages)) {
      ...
      conversation.messages.forEach((message, index) => {
        if (!message.id || typeof message.id !== 'string') {
          errors.push(
            `Conversation ${conversation.id}, Message ${index}: Invalid or missing ID`
          );
        }

        if (!message.content || typeof message.content !== 'string') {
          errors.push(
            `Conversation ${conversation.id}, Message ${index}: Invalid or missing content`
          );
        }
        ...
        if (
          ...
          errors.push(
            `Conversation ${conversation.id}, Message ${index}: Invalid role`
          );
        }
        ...
        if (
          ...
          errors.push(
            `Conversation ${conversation.id}, Message ${index}: Invalid timestamp`
          );
        }
      });
    }
    ...
    if (typeof conversation.isStreaming !== 'boolean') {
      errors.push(
        `Conversation ${conversation.id}: isStreaming must be a boolean`
      );
    }
    ...
    if (!Array.isArray(conversation.modelHistory)) {
      errors.push(
        `Conversation ${conversation.id}: modelHistory must be an array`
      );
    }

    if (!Array.isArray(conversation.compressionHistory)) {
      errors.push(
        `Conversation ${conversation.id}: compressionHistory must be an array`
      );
    }
  ...
  private repairConversation(conversation: Conversation): Conversation {
    ...
    // Fix invalid dates
    if (
      !(repaired.createdAt instanceof Date) ||
      isNaN(repaired.createdAt.getTime())
    ) {
      repaired.createdAt = new Date();
    }

    if (
      !(repaired.updatedAt instanceof Date) ||
      isNaN(repaired.updatedAt.getTime())
    ) {
