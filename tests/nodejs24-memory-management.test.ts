...
describe('Node.js 24 Memory Management', () => {
  ...
  describe('Enhanced Garbage Collection', () => {
    it('should handle large object allocation and cleanup efficiently', async () => {
      ...
      const largeObjects: Array<{ data: number[]; metadata: Record<string, unknown> }> = [];
      ...
      expect(afterAllocation.heapUsed).toBeGreaterThan(beforeAllocation.heapUsed);
  ...
  describe('Memory Pressure Handling', () => {
      ...
      const { metrics } = await measurePerformance(async () => {
        ...
        try {
          ...
          for (let i = 0; i < 100; i++) {
            ...
            if (currentSnapshot.heapUsed > initialSnapshot.heapUsed + 200 * 1024 * 1024) {
    ...
    it('should handle ArrayBuffer allocation efficiently', () => {
      ...
      expect(afterAllocation.arrayBuffers).toBeGreaterThan(beforeTest.arrayBuffers);
      ...
      expect(afterCleanup.external).toBeLessThanOrEqual(afterAllocation.external);
    });
  });
