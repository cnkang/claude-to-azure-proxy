...
test.describe('Diagnostic Tests', () => {
  test('should check if sidebar is visible and open', async ({ cleanPage, helpers }) => {
  ...
  test('should check if new conversation button works', async ({ cleanPage, helpers }) => {
    await helpers.waitForAppReady();
    ...
    if (buttonExists > 0) {
      ...
      if (isVisible) {
        ...
        const conversationsList = cleanPage.locator('[data-testid="conversations-list"]');
  ...
  test('should check if test conversation can be created', async ({ cleanPage, helpers }) => {
    await helpers.waitForAppReady();
    ...
    try {
      ...
      const conversationItem = await cleanPage.locator(
      ...
      await cleanPage.screenshot({ path: 'diagnostic-conversation-created.png' });
  ...
  test('should check if search component exists anywhere', async ({ cleanPage, helpers }) => {
    await helpers.waitForAppReady();
    ...
    console.log('Page contains "search":', bodyText?.toLowerCase().includes('search'));
  });
