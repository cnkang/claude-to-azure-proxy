...
type CrossTabSyncService = {
  readonly [K in BroadcastMethod]: (
...
async function instrumentCrossTabService(page: Page): Promise<void> {
  await page.evaluate(async (initialMonitor: SyncMonitor) => {
    ...
    const syncService = service as CrossTabSyncService;

    const wrapBroadcast = (method: BroadcastMethod) => {
      const original = syncService[method].bind(syncService);
      syncService[method] = (
        ...args: Parameters<CrossTabSyncService[typeof method]>
      ) => {
...
test.describe('App Context Persistence (E2E)', () => {
  test('hydrates persistence metadata and toggles search state from real browser interactions', async ({
    ...
    const searchStatus = await cleanPage.waitForSelector(
    ...
    const resultCount = await cleanPage.$$eval(
    ...
    await cleanPage.waitForSelector('.conversation-search-status', {
      state: 'hidden',
    });
  });
  ...
  test('syncs creation/update/delete across tabs and cleans up cross-tab listeners on unload', async ({
    ...
    const afterCreate = await cleanPage.evaluate(() => window.__syncMonitor);
    const broadcastCreated = afterCreate?.broadcasts?.some(
      (entry: { type: string; conversationId: string }) =>
        entry.type === 'broadcastCreation' &&
    ...
    const afterDelete = await cleanPage.evaluate(() => window.__syncMonitor);
    expect(
      afterDelete.broadcasts.some(
        (entry: { type: string; conversationId: string }) =>
          entry.type === 'broadcastDeletion' &&
          entry.conversationId === conversationId
      )
    ).toBe(true);

    await tab2.waitForTimeout(500);
    const tab2Conversation = await tab2.$(
