...
if (!AZURE_OPENAI_ENDPOINT || !AZURE_OPENAI_API_KEY || !AZURE_OPENAI_MODEL) {
  ...
  console.error(
    'Required: AZURE_OPENAI_ENDPOINT, AZURE_OPENAI_API_KEY, AZURE_OPENAI_MODEL'
  );
  process.exit(1);
}
...
async function testWithoutAbortController(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `[${new Date().toISOString()}] Response received (${requestDuration}ms)`
    );
      ...
      for (const line of lines) {
        ...
        if (line.startsWith('data: ')) {
          ...
          if (data === '[DONE]') {
            completed = true;
            console.log(
              `[${new Date().toISOString()}] Stream completed with [DONE] marker`
            );
            return;
          }
          ...
          try {
            ...
            if (parsed.type === 'response.output_text.delta' && parsed.delta) {
              ...
              if (parsed.response?.usage) {
                console.log(
                  `  Usage: ${JSON.stringify(parsed.response.usage)}`
                );
              }
            }
          } catch (parseError) {
    ...
    if (axios.isAxiosError(error)) {
      ...
      console.error(
        `  Is Canceled: ${error.code === 'ERR_CANCELED' || error.message.includes('canceled')}`
      );
      console.error(
        `  Is Timeout: ${error.code === 'ECONNABORTED' || error.message.includes('timeout')}`
      );
      if (error.response) {
        ...
        try {
          ...
          console.error(
            `  Response Data: [Unable to stringify - circular reference]`
          );
        }
      }
...
async function testWithAbortController(): Promise<void> {
  ...
  abortController.signal.addEventListener('abort', () => {
    console.log(
      `[${new Date().toISOString()}] ⚠️  AbortController signal aborted!`
    );
    console.log(
      `  Reason: ${abortController.signal.reason || 'No reason provided'}`
    );
  });
  ...
  try {
    console.log(
      `[${new Date().toISOString()}] Signal state before request: aborted=${abortController.signal.aborted}`
    );
    console.log(
      `[${new Date().toISOString()}] Making axios request with AbortController...`
    );
    ...
    console.log(`  Status: ${response.status} ${response.statusText}`);
    console.log(
      `  Signal state after response: aborted=${abortController.signal.aborted}`
    );
    console.log('');
    ...
    response.data.on('data', (chunk: Buffer) => {
      ...
      if (chunkCount === 1 || chunkCount % 10 === 0) {
        console.log(
          `[${new Date().toISOString()}] Chunk ${chunkCount}: signal.aborted=${abortController.signal.aborted}`
        );
      }
      ...
      for (const line of lines) {
        ...
        if (line.startsWith('data: ')) {
          ...
          if (data === '[DONE]') {
            ...
            console.log(
              `  Signal state: aborted=${abortController.signal.aborted}`
            );
            return;
          }
    ...
    console.error(
      `  Signal state at error: aborted=${abortController.signal.aborted}`
    );
    console.error(
      `  Signal reason: ${abortController.signal.reason || 'No reason'}`
    );
...
async function testMinimalConfiguration(): Promise<void> {
  ...
  try {
    console.log(
      `[${new Date().toISOString()}] Making minimal axios request...`
    );
      ...
      for (const line of lines) {
        ...
        if (line.startsWith('data: ')) {
          ...
          if (data === '[DONE]') {
            completed = true;
            console.log(
              `[${new Date().toISOString()}] Stream completed with [DONE]`
            );
            return;
          }
...
async function main(): Promise<void> {
  ...
  console.log('');
  console.log(
    'This script tests direct axios calls to Azure OpenAI Responses API'
  );
  console.log(
    'to isolate the "canceled" error and compare behavior with/without'
  );
  ...
  try {
    ...
    console.log(
      '  - Test 3: Minimal configuration (no timeout, no AbortController)'
    );
    console.log('');
    console.log(
      'Review the output above to identify any differences in behavior.'
    );
    console.log('');
  } catch (error) {
