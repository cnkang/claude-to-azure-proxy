...
describe('Node.js 24 Features', () => {
  describe('Runtime Environment', () => {
    it('should be running on Node.js 24+', () => {
      ...
      if (majorVersion < 24) {
        console.warn(`Node.js 24 validation warning: Node.js 24+ required, but running v${process.version}`);
    ...
    it('should have V8 13.6+ engine', () => {
      ...
      const v8MajorVersion = parseInt(v8Version.split('.')[0], 10);
      const v8MinorVersion = parseInt(v8Version.split('.')[1], 10);
  ...
  describe('Memory Management', () => {
    ...
    it('should detect memory usage patterns', async () => {
      ...
      expect(afterAllocationSnapshot.heapUsed).toBeGreaterThan(initialSnapshot.heapUsed);
      ...
      const memoryDifference = afterCleanupSnapshot.heapUsed - afterAllocationSnapshot.heapUsed;
  ...
  describe('Async Context Tracking', () => {
    it('should support AsyncResource for context tracking', async () => {
      ...
      const testFunction = createTestAsyncResource('test-context', (value: string) => {
        contextValue = value;
    ...
    it('should maintain context across async operations', async () => {
      ...
      const asyncOperation = createTestAsyncResource('async-op', async (id: string) => {
      ...
      expect(contextValues.filter(v => v.startsWith('start-'))).toHaveLength(3);
  ...
  describe('Performance Monitoring', () => {
    ...
    it('should handle timeout scenarios', async () => {
      ...
      await expect(withTimeout(slowOperation, 50)).rejects.toThrow('Test timed out after 50ms');
    });
