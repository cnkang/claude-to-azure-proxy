...
class StreamingFlowValidator {
  ...
  public async runAllTests(): Promise<void> {
    ...
    await this.runTest(3, 'Consecutive Request #1', 'What is 2 + 2?');
  ...
  private async runTest(
    ...
    try {
          ...
          if (line.startsWith('data: ')) {
            ...
            try {
              ...
              if (parsed.type === 'start') {
                ...
                console.log(
                  `[START] Message ID: ${parsed.messageId}, Model: ${parsed.model}`
                );
                ...
                if (chunksReceived <= 3 || chunksReceived % 5 === 0) {
                  console.log(
                    `[CHUNK ${chunksReceived}] ${content.length} chars: "${content.substring(0, 50)}${content.length > 50 ? '...' : ''}"`
                  );
                }
                ...
                if (parsed.usage) {
                  usageReturned = true;
                  console.log(
                    `[END] Usage: input=${parsed.usage.inputTokens}, output=${parsed.usage.outputTokens}, total=${parsed.usage.totalTokens}`
                  );
                } else {
              ...
              console.warn(
                `Failed to parse SSE data: ${data.substring(0, 100)}`
              );
            }
          }
        }
      ...
      console.log(
        `  Canceled error: ${canceledError ? '❌ FAILED' : '✅ None'}`
      );
        ...
        if (error.response?.data) {
          try {
            if (typeof error.response.data === 'string') {
              ...
              console.log(
                `  API Error: ${JSON.stringify(error.response.data.error, null, 2)}`
              );
            }
          } catch (e) {
  ...
  private printSummary(): void {
    ...
    console.log(
      `Canceled Errors: ${canceledTests} ${canceledTests > 0 ? '❌' : '✅'}`
    );
    console.log();
    ...
    for (const result of this.results) {
      ...
      console.log(
        `${result.testNumber.toString().padStart(4)} | ${name} | ${status} | ${result.chunksReceived.toString().padStart(6)} | ${usage.padStart(5)} | ${canceled}`
      );
    }
    console.log('-'.repeat(80));
    console.log();
    ...
    const totalChunks = this.results.reduce(
      (sum, r) => sum + r.chunksReceived,
      0
    );
    const totalContent = this.results.reduce(
    ...
    console.log(
      `  ✓ All tests passed: ${passedTests === totalTests ? '✅ YES' : '❌ NO'}`
    );
    console.log(
      `  ✓ No canceled errors: ${canceledTests === 0 ? '✅ YES' : '❌ NO'}`
    );
    console.log(
      `  ✓ All chunks received: ${totalChunks > 0 ? '✅ YES' : '❌ NO'}`
    );
    console.log(
      `  ✓ Usage statistics returned: ${this.results.every((r) => r.usageReturned || !r.success) ? '✅ YES' : '❌ NO'}`
    );
    console.log(
      `  ✓ Multiple consecutive requests: ${totalTests >= 3 ? '✅ YES' : '❌ NO'}`
    );
    console.log();
