...
export function SessionProvider({ children }: SessionProviderProps): React.JSX.Element {
  ...
  useEffect(() => {
    if (!sessionHook.isLoading) {
      ...
      if (!sessionHook.storageAvailable.sessionStorage) {
        setError('Session storage is not available. Some features may not work properly.');
      } else if (!sessionHook.storageAvailable.localStorage) {
        setError('Local storage is not available. Preferences will not be saved.');
      } else {
        setError(null);
      }
    }
  ...
  useEffect(() => {
    if (!sessionHook.isSessionValid && isInitialized) {
      setError('Session validation failed. Your session has been reset for security.');
...
export function SessionGuard({ 
  ...
  if (requireValidSession !== null && requireValidSession !== undefined && (!isSessionValid || session === null)) {
    return <>{fallback ?? null}</>;
  }
...
export function SessionDebugInfo({ className }: SessionDebugInfoProps): React.JSX.Element | null {
  const { session, sessionId, isSessionValid, storageUsage } = useSessionContext();
  ...
  return (
    <div className={className} style={{ 
      ...
      <div><strong>Session Debug Info:</strong></div>
      ...
      <div>Storage: {storageUsage.used} / {storageUsage.quota} bytes</div>
    </div>
  );
}
