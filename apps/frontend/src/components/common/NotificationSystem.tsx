...
interface NotificationContextType {
  ...
  showError: (_error: Error | NetworkError | string, options?: Partial<Notification>) => string;
...
export function NotificationProvider({ 
  ...
  const showError = useCallback((
    ...
    if (error instanceof NetworkError) {
      ...
      message = t(`error.network.${error.type}`, { defaultValue: error.message });
...
function NotificationItem({ notification }: NotificationItemProps): React.JSX.Element {
  const context = useContext(NotificationContext);
  const { t } = useTranslation();
  const [isVisible, setIsVisible] = useState(false);

  if (!context) {
    throw new Error('NotificationItem must be used within NotificationProvider');
  }
              ...
              {notification.actions.map((action, index) => (
                <button
                  ...
                  style={{
                    ...
                    backgroundColor: action.primary ? getIconColor() : 'transparent',
...
export function useErrorNotification() {
  ...
  return useCallback((error: Error | NetworkError | string, options?: Partial<Notification>) => {
