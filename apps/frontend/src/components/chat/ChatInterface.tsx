...
import { getChatService, chatUtils, type SSEConnectionState } from '../../services/chat';
...
export const ChatInterface = memo<ChatInterfaceProps>(({
  ...
  const sseConnectionRef = useRef(getChatService().getSSEConnection(conversation.id));

  /**
   * Handle sending a new message
   */
  const handleSendMessage = useCallback(async (content: string, files?: File[]): Promise<void> => {
    ...
    try {
      ...
      const userMessage = chatUtils.createUserMessage(content, conversation.id, fileInfos);
      ...
      const streamingMsg = chatUtils.createStreamingMessage(messageId, conversation.id, correlationId);
  ...
  }, [conversation, chatService, updateConversation, onConversationUpdate, t, announce]);

  /**
   * Handle streaming message updates
   */
  const handleMessageChunk = useCallback((data: { content: string; messageId: string; correlationId: string }): void => {
   ...
   */
  const handleMessageEnd = useCallback((data: { messageId: string; correlationId: string }): void => {
    ...
    setIsLoading(false);
  }, [streamingMessage, conversation, updateConversation, onConversationUpdate, announce]);

  /**
   * Handle streaming errors
   */
  const handleMessageError = useCallback((data: { _error: string; correlationId: string }): void => {
   ...
   */
  const handleConnectionStateChange = useCallback((state: SSEConnectionState): void => {
    if (state === 'error') {
  ...
  const handleRetryMessage = useCallback((messageId: string): void => {
    ...
    const message = conversation.messages.find((candidate: Message) => candidate.id === messageId);
    if (message?.role === 'user') {
      const clearedRetryConversation: Conversation = {
        ...conversation,
        messages: conversation.messages.map((candidate: Message) =>
          candidate.id === messageId ? { ...candidate, retryable: false } : candidate
        ),
      };
      updateConversation(clearedRetryConversation.id, clearedRetryConversation);
      ...
      const files = message.files?.map((fileInfo: FileInfo) => 
        new File([], fileInfo.name, { type: fileInfo.type })
      );
      handleSendMessage(message.content, files).catch((retryError: unknown) => {
  ...
  }, [conversation, handleSendMessage, onConversationUpdate, updateConversation]);
      ...
      >
        ...
        <header className="chat-header" role="banner">
          <div className="conversation-info">
            ...
            <div className="conversation-meta" role="status" aria-live="polite">
              <span className="model-badge" aria-label={t('chat.currentModel', { model: conversation.selectedModel })}>
                {conversation.selectedModel}
              </span>
              <span className="message-count" aria-label={t('chat.messageCount', { count: conversation.messages.length })}>
                {t('chat.messageCount', { count: conversation.messages.length })}
              </span>
              {conversation.contextUsage ? (
                <span 
                  ...
                  aria-label={t('context.usage.tooltip', {
                    ...
                    percentage: Math.round((conversation.contextUsage.currentTokens / conversation.contextUsage.maxTokens) * 100)
                  })}
                >
                  {Math.round((conversation.contextUsage.currentTokens / conversation.contextUsage.maxTokens) * 100)}%
                </span>
              ) : null}
            </div>
          </div>
        ...
        {/* Message Input */}
        <section className="chat-input" role="form" aria-label={t('chat.messageInput')}>
          <MessageInput
