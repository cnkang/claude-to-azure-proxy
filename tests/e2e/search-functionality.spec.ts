...
test.describe('Search Functionality', () => {
  test('should search and display correct result count with highlighting', async ({ cleanPage, helpers }) => {
    // Create test conversations with searchable content
    await helpers.createTestConversation(
  ...
  test('should highlight keywords in search results with proper context', async ({ cleanPage, helpers }) => {
    ...
    await helpers.createTestConversation(
      ...
      [
        {
          role: 'user',
          content: 'How do I integrate the payment API into my application? I need detailed steps.',
        },
        {
          role: 'assistant',
          content: 'To integrate the payment API, first obtain your API credentials from the dashboard.',
        },
      ]
    );
  ...
  test('should handle pagination with 20 results per page', async ({ cleanPage, helpers }) => {
  ...
  test('should handle empty search results with helpful message', async ({ cleanPage, helpers }) => {
    ...
    const noResults = await cleanPage.waitForSelector(
      '.no-results',
      { state: 'visible', timeout: 2000 }
    );
  ...
  test('should support case-insensitive search by default', async ({ cleanPage, helpers }) => {
  ...
  test('should support case-sensitive search when enabled', async ({ cleanPage, helpers }) => {
    ...
    const caseSensitiveToggle = await cleanPage.waitForSelector(
      'input[type="checkbox"][aria-label*="case-sensitive"]',
      { state: 'visible', timeout: 2000 }
    ).catch(() => null);
  ...
  test('should complete search within 500ms', async ({ cleanPage, helpers }) => {
    ...
    if (timeMatch) {
      const displayedTime = parseInt(timeMatch[1], 10);
      expect(displayedTime).toBeLessThan(500); // Actual search time should be <500ms
    }
  });
  
  test('should open conversation and scroll to first keyword occurrence', async ({ cleanPage, helpers }) => {
    ...
    const conversationId = await helpers.createTestConversation(
      ...
      [
        ...
        { role: 'assistant', content: 'Third response about the important topic' },
      ]
    );
    ...
    const firstResult = await cleanPage.waitForSelector('[data-testid^="search-result-"]');
    ...
    const conversationHighlights = await cleanPage.$$('.conversation-view mark');
  ...
  test('should provide navigation controls to jump between keyword occurrences', async ({ cleanPage, helpers }) => {
    ...
    await helpers.createTestConversation(
      ...
      [
        ...
        { role: 'assistant', content: 'React hooks let you use state in function components' },
      ]
    );
    ...
    const highlightNav = await cleanPage.waitForSelector('.highlight-navigation');
    const prevButton = await highlightNav.waitForSelector('button[aria-label*="Previous"]');
    const nextButton = await highlightNav.waitForSelector('button[aria-label*="Next"]');
    const counter = await highlightNav.waitForSelector('span[aria-live="polite"]');
  ...
  test('should maintain search index when conversations are updated', async ({ cleanPage, helpers }) => {
    ...
    let searchInput = await cleanPage.waitForSelector('input[role="searchbox"]');
  ...
  test('should maintain search index when conversations are deleted', async ({ cleanPage, helpers }) => {
  ...
  test('should prefetch first 3 pages for faster navigation', async ({ cleanPage, helpers }) => {
    ...
    const nextButton = await cleanPage.waitForSelector('button[aria-label="Next page"]');
  ...
  test('should handle keyboard navigation in search results', async ({ cleanPage, helpers }) => {
  ...
  test('should display search statistics with result count and time', async ({ cleanPage, helpers }) => {
  ...
  test('should handle search with multiple keywords', async ({ cleanPage, helpers }) => {
