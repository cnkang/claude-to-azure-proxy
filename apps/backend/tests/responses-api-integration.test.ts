...
// Helper function to get Express app from ProxyServer
const getAppFromServer = (server: ProxyServer): Application => {
  return server.getApp();
};

// Helper function to create test server
const createTestServer = (): ProxyServer => {
...
const withAuth = <T extends SupertestTest>(req: T): T => {
  return req
    .set('Authorization', `Bearer ${testConfig.PROXY_API_KEY}`)
    .set('x-api-key', testConfig.PROXY_API_KEY);
};

describe('Responses API Integration Tests', () => {
  let app: Application;
  let server: ProxyServer;
  let previousRateLimitTestMaxRequests: string | undefined;

  beforeAll(async () => {
    previousRateLimitTestMaxRequests = process.env.RATE_LIMIT_TEST_MAX_REQUESTS;
    process.env.RATE_LIMIT_TEST_MAX_REQUESTS = '10000';

    const indexModule = await import('../src/index.js');
    ProxyServer = indexModule.ProxyServer;

    // Create test server with proper configuration
    server = createTestServer();
    app = getAppFromServer(server);
  ...
  describe('Claude Format Compatibility', () => {
    it('should handle Claude format requests with full request-response cycle', async () => {
      ...
      const response = await withAuth(request(app).post('/v1/messages'))
        .send(claudeRequest)
        .expect(200);
  ...
  describe('OpenAI Format Compatibility', () => {
    it('should handle OpenAI format requests with full request-response cycle', async () => {
      ...
      const response = await withAuth(request(app).post('/v1/chat/completions'))
        .send(openaiRequest)
        .expect(200);
  ...
  describe('Conversation Continuity', () => {
    it('should maintain conversation context across multiple requests', async () => {
      ...
      const response1 = await withAuth(request(app).post('/v1/messages'))
      ...
      const response2 = await withAuth(request(app).post('/v1/messages'))
        .set('X-Conversation-ID', conversationId)
        .send(secondRequest)
        .expect(200);
    ...
    it('should track conversation metrics', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        await withAuth(request(app).post('/v1/messages'))
          .set('X-Conversation-ID', conversationId)
          .send(requestData)
          .expect(200);
      }
  ...
  describe('Error Scenarios and Graceful Degradation', () => {
    ...
    it('should handle timeout scenarios', async () => {
      ...
      const response = await withAuth(request(app).post('/v1/messages')).send(
        claudeRequest
      );
