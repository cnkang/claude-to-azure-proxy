...
test.describe('Component Rendering Order', () => {
  test('should verify component rendering order and storage availability', async ({ cleanPage, helpers }) => {
    ...
    const testBridgeAvailable = await cleanPage.evaluate(() => {
      return {
        hasBridge: typeof (window as any).__TEST_BRIDGE__ !== 'undefined',
        hasStorage: typeof (window as any).__TEST_BRIDGE__?.getConversationStorage === 'function',
        hasSessionManager: typeof (window as any).__TEST_BRIDGE__?.getSessionManager === 'function',
        e2eMode: (window as any).__E2E_TEST_MODE__,
      };
    });
    
    console.log('Test Bridge Status:', JSON.stringify(testBridgeAvailable, null, 2));
    ...
    const renderingOrder = await cleanPage.evaluate(() => {
      const components = {
        appContainer: document.querySelector('[data-testid="app-container"]') !== null,
        sidebar: document.querySelector('[data-testid="sidebar"]') !== null,
        conversationsList: document.querySelector('[data-testid="conversations-list"]') !== null,
        newConversationButton: document.querySelector('[data-testid="new-conversation-button"]') !== null,
    ...
    console.log('Component Rendering Status:', JSON.stringify(renderingOrder, null, 2));
    ...
    console.log('Available Test IDs:', JSON.stringify(availableTestIds, null, 2));
  ...
  test('should verify conversation creation flow', async ({ cleanPage, helpers }) => {
    ...
    const newButton = await cleanPage.locator('[data-testid="new-conversation-button"]');
    ...
    if (!buttonVisible) {
      ...
      const buttonInfo = await cleanPage.evaluate(() => {
        const button = document.querySelector('[data-testid="new-conversation-button"]');
    ...
    console.log('Conversation Creation Result:', JSON.stringify(conversationCreated, null, 2));
    
    // Step 5: Check if conversation appears in UI
    console.log('\nStep 5: Checking if conversation appears in UI...');
    const conversationItems = await cleanPage.locator('[data-testid^="conversation-item-"]').count();
  ...
  test('should check browser console for errors', async ({ cleanPage, helpers }) => {
