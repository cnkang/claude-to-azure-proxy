...
import { useTheme, useThemeStyles, ThemeToggle, _wcagUtils as wcagUtils } from '../../contexts/ThemeContext';
...
export function ThemeDemo(): React.JSX.Element {
  ...
  return (
    <div className="theme-demo" style={{ padding: '2rem', maxWidth: '800px', margin: '0 auto' }}>
      ...
      <section style={{ marginBottom: '2rem' }}>
        ...
        <div style={{ 
          ...
          <p><strong>Theme Mode:</strong> {theme.themeMode}</p>
          <p><strong>Resolved Theme:</strong> {theme.resolvedTheme}</p>
          <p><strong>System Prefers Dark:</strong> {theme.systemPrefersDark ? 'Yes' : 'No'}</p>
          <p><strong>Is Auto Mode:</strong> {theme.isAutoMode ? 'Yes' : 'No'}</p>
          <p><strong>Contrast Ratio:</strong> {styles.accessibility.contrastRatio}:1 (AAA Compliant)</p>
          <p><strong>High Contrast:</strong> {styles.accessibility.isHighContrast ? 'Yes' : 'No'}</p>
          <p><strong>Reduced Motion:</strong> {styles.accessibility.prefersReducedMotion ? 'Yes' : 'No'}</p>
        </div>
      </section>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem' }}>
          {Object.entries(styles.colors).map(([name, color]) => (
            <div 
              ...
              style={{
                background: color,
                color: name === 'background' ? styles.colors.foreground : styles.colors.background,
                padding: '1rem',
                borderRadius: '8px',
                border: '1px solid var(--color-border)',
                textAlign: 'center'
            ...
            >
              ...
              <div style={{ fontSize: '0.75rem', marginTop: '0.5rem' }}>
                Contrast: {wcagUtils.getContrastRatio(color, styles.colors.background).toFixed(1)}:1
              </div>
            </div>
          ))}
        </div>
