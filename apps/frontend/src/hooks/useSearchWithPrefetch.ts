...
import type { ConversationSearchService, SearchResponse } from '../services/conversation-search';
...
export function useSearchWithPrefetch(
   ...
   * Check if result is cached
   */
  const isCached = useCallback((query: string, page: number): boolean => {
   ...
   */
  const evictOldestEntries = useCallback((newCache: Map<string, SearchResponse>) => {
  ...
  const searchWithPrefetch = useCallback(async (
    ...
    return response;
  }, [cache, getCacheKey, searchService, evictOldestEntries, initialPages, prefetchPages, debug]);
